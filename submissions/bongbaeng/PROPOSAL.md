...
## 8. Update v2 — ครบทั้งสาม (จาก cohort review DustBoy/Jizo/No.6)
Cohort สรุป "แข็งสุด = topic-in-signed-body + EIP-712 จริง + persisted seq" — proposal นี้อัปเป็นครบทั้งสาม:
```
1. topic-in-signed-body  ✅ topic อยู่ใน signed message + verify เทียบ deliveryTopic (กัน broker reroute)
2. EIP-712 จริง (default) ✅ signTypedData/recoverTypedDataAddress · domain ผูก chainId 20260619
                            → cross-chain/domain replay กันที่ระดับ crypto (ไม่ใช่ string prefix)
                            → wallet โชว์ field (กัน blind-sign)
3. persisted seq         ✅ seq เก็บใน CF KV/Durable Object (ไม่ใช่ in-memory Map)
                            → restart/scale แล้ว replay protection ไม่พัง (cohort weak#1)
```
ดู `examples/verify-worker.ts` + `examples/client-sign.ts` (EIP-712 default แล้ว)
