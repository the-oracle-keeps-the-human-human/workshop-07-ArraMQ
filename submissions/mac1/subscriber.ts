...
client.on('message', async (topic, payload) => {
  ...
  try {
    ...
    // Perform end-to-end EIP-712 signature and freshness check
    const isValid = await verifyTelemetryMessage(envelope);
    ...
    if (isValid) {
      ...
      console.error(`[Subscriber] ❌ SIGNATURE / FRESHNESS CHECK FAILED!`);
    }
  } catch (error) {
